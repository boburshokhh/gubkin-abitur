...
INSERT INTO directions (id, level_id, code, name, sort_order, is_published) VALUES
  (1, 2, '21.05.03', 'Технология геологической разведки', 10, TRUE),
  (2, 1, '21.03.01', 'Нефтегазовое дело', 20, TRUE),
  (3, 2, '21.05.06', 'Нефтегазовые техника и технологии', 30, TRUE),
  (4, 1, '38.03.01', 'Экономика', 40, TRUE),
  (5, 1, '38.03.02', 'Менеджмент', 50, TRUE)
ON CONFLICT (id) DO UPDATE SET level_id = EXCLUDED.level_id, code = EXCLUDED.code, name = EXCLUDED.name, sort_order = EXCLUDED.sort_order, is_published = EXCLUDED.is_published;

-- Профили подготовки
INSERT INTO profiles (id, direction_id, name, description, duration_years, tuition_fee, places, sort_order, is_published) VALUES
  -- 21.05.03
  (1, 1, 'Цифровой геоинжиниринг (УГЦ)', 'Подготовка специалистов в области цифрового моделирования месторождений.', 5.0, 'Контракт', 30, 10, TRUE),
  -- 21.03.01
  (2, 2, 'Бурение нефтяных и газовых скважин (УРБ)', 'Профиль подготовки специалистов по бурению скважин на суше и море.', 4.0, 'Контракт', 30, 10, TRUE),
  (3, 2, 'Эксплуатация и обслуживание объектов добычи нефти (УРН)', 'Обучение методам разработки и эксплуатации нефтяных месторождений.', 4.0, 'Контракт', 30, 20, TRUE),
  (4, 2, 'Эксплуатация и обслуживание объектов добычи газа, газоконденсата и подземных хранилищ (УРГ)', 'Подготовка кадров для газодобывающего сектора.', 4.0, 'Контракт', 30, 30, TRUE),
  (5, 2, 'Эксплуатация и обслуживание объектов транспорта и хранения нефти, газа и продуктов переработки (УТП)', 'Обучение проектированию и эксплуатации трубопроводов и хранилищ.', 4.0, 'Контракт', 30, 40, TRUE),
  -- 21.05.06
  (6, 3, 'Технология бурения нефтяных и газовых скважин на суше и море (УРТ)', 'Глубокая подготовка инженеров-буровиков.', 5.0, 'Контракт', 30, 10, TRUE),
  (7, 3, 'Разработка и эксплуатация нефтяных месторождений (УРС)', 'Проектирование и управление процессом нефтедобычи.', 5.0, 'Контракт', 30, 20, TRUE),
  (8, 3, 'Разработка и эксплуатация газовых и газоконденсатных месторождений (УРИ)', 'Управление процессами добычи газа.', 5.0, 'Контракт', 30, 30, TRUE),
  -- 38.03.01
  (9, 4, 'Экономика и проекты устойчивого развития энергетики (УЭЭ)', 'Подготовка экономистов для ТЭК.', 4.0, 'Контракт', 30, 10, TRUE),
  -- 38.03.02
  (10, 5, 'Управление бизнесом в энергетике (УЭМ)', 'Подготовка менеджеров и аналитиков.', 4.0, 'Контракт', 30, 10, TRUE),
  (11, 5, 'Международный менеджмент (УБМ)', 'Подготовка управленцев для международных энергетических проектов.', 4.0, 'Контракт', 30, 20, TRUE)
ON CONFLICT (id) DO UPDATE SET direction_id = EXCLUDED.direction_id, name = EXCLUDED.name, description = EXCLUDED.description, duration_years = EXCLUDED.duration_years, places = EXCLUDED.places, sort_order = EXCLUDED.sort_order, is_published = EXCLUDED.is_published;
